  ...
  methods: {
    ...
    async open() {
      ...
      if (this.payInfo.codeUrl) {
        let erwei = await QRCode.toDataURL(this.payInfo.codeUrl);
        // console.log(erwei);
        this.$alert(`<img src=${erwei} />`, "请你微信支付", {
          ...
          showClose: false,
          // action 的值为'confirm', 'cancel'或'close'；instance 为 MessageBox 实例，可以通过它访问实例上的属性和方法；done 用于关闭 MessageBox 实例
          beforeClose: (action, instance, done) => {
            if (action == "cancel") {
              ...
              if (this.code == 200) {
                ...
                // 同样跳转到 支付成功页面, 只是测试 新增入口

                alert("抱歉,暂时未收到付款");

                // 下面的代码注释掉就能够防止点击 已成功支付进行跳转
                done(); // 关闭窗口
                clearInterval(this.timer); // 清楚定时器
                this.timer = null; // 清空内存
                this.$router.push("/paysuccess"); // 跳转到支付成功页面
              }
            }
          },
        })
